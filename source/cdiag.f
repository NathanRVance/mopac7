...
C TO FIND THE EIGENVALUES AND EIGENVECTORS OF A HERMITIAN MATRIX.
      REAL VALUE(*)
      COMPLEX W1(2000), W2(2000), W3(2000)
      COMPLEX A(N,*),VEC(N,*)
      REAL W3R(4000)
      EQUIVALENCE (W3R,W3)
      IA=N
...
C REDUCE MATRIX TO A TRI-DIAGONAL HERMITIAN MATRIX.
      CALL ME08A(A,W1,W2,N,IA,W3)
C
C FIND THE EIGENVALUES AND EIGENVECTORS OF THE TRI-DIAGONAL MATRIX
      CALL EC08C(W1,W2,VALUE,VEC,N,IV,W3R)
      CALL SORT(VALUE,VEC,N)
      RETURN
      END
...
C                       (4)
         DO 50 K=I1,N
   50    CALL ME08B (A(K,K),Q(K),A(I,K),N-K+1,IA)
   60 CONTINUE
      DO 70 I=2,N
         QJ=ALPHA(I)
         ALPHA(I)=A(I,I)
   ...
   90 RETURN
      ...
      COMPLEX A(IA,*),Q(*),B(IA,*)
      DO 10 I=1,N
         A(1,I)=A(1,I) - (Q(1)*B(1,I)) - CONJG(Q(I)*B(1,1))
   10 CONTINUE
      RETURN
      END
...
C  HERMITIAN MATRIX.
      REAL VALUE(*),W(*),ONE,ZERO
      COMPLEX A(*),B(*),VEC(*),DN,UPCK
...
C  FIND THE EIGENVALUES AND VECTORS OF THE REAL FORM
   30 CALL EA08C(W,W(N+1),VALUE,VEC,N,IV,W(N2+1))
CC
CC THE VECTORS IN VEC AT THIS POINT ARE REAL,WE NOW EXPAND THEM
CC INTO VEC AS THOUGH THEY WERE COMPLEX.
C      DO 50 I=1,IL,IV2
C         DO 40 J=1,N
C            K=N-J
C            L=K+K
C   40    VEC(I+L)=VEC(I+K)
C   50 VEC(I+1)=ZERO
      IF(N.LE.1)GO TO 80
      DN=ONE
      K=1
...
C     RAYLEIGH QUOTIENT
      DO 120 J=1,M
         ...
         DO 110 I=2,M
            KI=K+I
            XX=XX+REAL(VEC(KI))**2
  110    XAX=XAX+REAL(VEC(KI))*(2.*B(I)*REAL(VEC(KI-1))+A(I)*
     1       REAL(VEC(KI)))
  120 VALUE(J)=XAX/XX
      RETURN
      END
